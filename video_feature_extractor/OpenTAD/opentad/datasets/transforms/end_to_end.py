...
class LoadFrames:
    ...
    def __call__(self, results):
        ...
        if self.method == "resize":
            ...
            # pad the frame_idxs
            if len(frame_idxs) < frame_num:
                valid_len = len(frame_idxs)
                frame_idxs = np.pad(frame_idxs, (0, frame_num - valid_len), mode="edge")
                masks = torch.cat([torch.ones(valid_len), torch.zeros(frame_num - valid_len)]).bool()
            else:
                masks = torch.ones(frame_num).bool()
