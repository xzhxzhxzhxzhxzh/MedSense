...
def eval_metrics(dvc_filename, gt_filenames, para_gt_filenames, alpha=0.3, temperature=2.0, cl_score_weight=0., ranking_key='proposal_score', rerank=False, dvc_eval_version='2018', verbose=False):
    ...
    dvc_score = {k: {"mean": np.mean(v), "tiou@0.3": v[0], "tiou@0.5": v[1], "tiou@0.7": v[2]} for k, v in dvc_score.items()}
    ...
    # dvc_score.update({'MetaScore': dvc_score['METEOR']["mean"] + dvc_score['soda_c']})
...
def evaluate(model, criterion, contrastive_criterion, postprocessors, loader, dvc_json_path, logger=None, score_threshold=0,
    ...
    with torch.set_grad_enabled(False):
        for dt in tqdm(loader, disable=opt.disable_tqdm):
            ...
            output, loss = model(dt, criterion, contrastive_criterion, eval_mode=True)
            ...
            for key in weight_dict.keys():
                ...
                try:
                    ...
                    not_eval_loss_keys.append(key)

                if ext in ("enc", "dn"):
                    not_eval_loss_keys.append(key)

            final_loss = sum(loss[k] * weight_dict[k] for k in loss.keys() if (k in weight_dict) and (k not in not_eval_loss_keys))
