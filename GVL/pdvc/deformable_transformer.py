...
class DeformableTransformer(nn.Module):
    def __init__(self, d_model=256, nhead=8,
                 ...
                 two_stage_mode=None, two_stage_num_proposals=100, look_forward_twice=False):
        super().__init__()
    ...
    def prepare_encoder_inputs(self, srcs, masks, pos_embeds):
        ...
        level_start_index = torch.cat((temporal_shapes.new_zeros((1,)), temporal_shapes.cumsum(0)[:-1]))  # prod: [w0h0, w0h0+w1h1, w0h0+w1h1+w2h2, ...]
        valid_ratios = torch.stack([self.get_valid_ratio(m) for m in masks], 1)  # (bs, lvl_num, 2), where 2 means (h_rate, and w_rate)， all values <= 1
    ...
    def prepare_decoder_input_proposal(self, memory, memory_padding_mask, spatial_shapes, refpoint_embed, tgt):
        N_, _, _ = memory.shape
        proposals = []
        _cur = 0
        for lvl, S_ in enumerate(spatial_shapes):
            # get spatial width of each scale level of the base-encoder
            mask_flatten_ = memory_padding_mask[:, _cur:(_cur + S_)]
            valid_S = torch.sum(~mask_flatten_, 1)

            center = torch.linspace(0, S_ - 1, S_, dtype=torch.float32, device=memory.device)
            scale = valid_S.expand(N_, 1)
            center = (center.expand(N_, -1) + 0.5) / scale
            width = torch.ones_like(center) * 0.05 * (2.0 ** lvl)
            proposal = torch.cat((center.unsqueeze(-1), width.unsqueeze(-1)), -1).view(N_, -1, 2)
        ...
        enc_outputs_class = self.decoder.class_head[self.decoder.num_layers](output_memory)
        ...
        topk_memory_unact = torch.gather(output_memory, 1, topk_proposals.unsqueeze(-1).repeat(1, 1, self.d_model))
        if self.two_stage_mode == "e2e":
            topk_coords_unact = topk_coords_unact
        else:
            topk_coords_unact = topk_coords_unact.detach()

        if refpoint_embed is not None:
            # cdn + coords proposal
            refpoint_embed = torch.cat([refpoint_embed, topk_coords_unact], dim=1)
            reference_points = refpoint_embed.sigmoid()
            init_reference_out = reference_points

            pos_trans_out = self.pos_trans_norm(self.pos_trans(self.get_proposal_pos_embed(refpoint_embed)))
            refpoint_embed, tgt_ = torch.chunk(pos_trans_out, 2, dim=2)

            dn_num = tgt.shape[1]
            tgt_ = torch.cat((tgt, tgt_[:, dn_num:, :]), dim=1)
            tgt = tgt_
        else:
            # inference mode, no cdn
            reference_points = topk_coords_unact.sigmoid()
            init_reference_out = reference_points
            pos_trans_out = self.pos_trans_norm(self.pos_trans(self.get_proposal_pos_embed(topk_coords_unact)))
            refpoint_embed, tgt = torch.chunk(pos_trans_out, 2, dim=2)

        if self.two_stage_mode == "e2e":
            raise NotImplementedError
        else:
            pred_logits = self.decoder.class_head[self.decoder.num_layers](topk_memory_unact)
            ...
            return init_reference_out, tgt, reference_points, refpoint_embed, enc_outputs
...
class DeformableTransformerDecoder(nn.Module):
    ...
    def forward(self, tgt, reference_points, memory, temporal_shapes, level_start_index, valid_ratios,
                query_embed, memory_key_padding_mask, attn_mask, disable_iterative_refine=False):
        output = tgt
        ...
        for lid, layer in enumerate(self.layers):
            if reference_points.shape[-1] == 2:
                reference_points_input = reference_points[:, :, None] \
                                         * torch.stack([valid_ratios, valid_ratios], -1)[:, None]
            else:
                assert reference_points.shape[-1] == 1
                reference_points_input = reference_points[:, :, None] * valid_ratios[:, None, :, None]

            output = layer(output, query_embed, reference_points_input, memory, temporal_shapes, level_start_index, 
