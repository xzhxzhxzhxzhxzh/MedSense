...
class SetCriterion(nn.Module):
    ...
    def loss_labels(self, outputs, targets, indices, num_boxes, log=True):
        ...
        target_classes[idx] = target_classes_o  # zero is positive

        target_classes_onehot = torch.zeros([src_logits.shape[0], src_logits.shape[1], src_logits.shape[2] + 1],
                                            dtype=src_logits.dtype, layout=src_logits.layout, device=src_logits.device)
        target_classes_onehot.scatter_(2, target_classes.unsqueeze(-1), 1)  # in the first column, 1 is true, 0 is false
    ...
    def forward(self, outputs, targets):
        ...
        """
        outputs_without_aux = {k: v for k, v in outputs.items() if k not in ("aux_outputs", "enc_outputs", "dn_outputs", "dn_aux_outputs")}
        device = next(iter(outputs.values())).device
        mat_indices = self.matcher(outputs_without_aux, targets)
        outputs["matched_indices"] = mat_indices
        ...
        for loss in self.losses:
            kwargs = {}
            losses.update(self.get_loss(loss, outputs, targets, mat_indices, num_boxes, **kwargs))
        ...
        if "enc_outputs" in outputs:
            ...
            enc_indices = self.matcher(enc_outputs, bin_targets, proposal_out=True)
            for loss in self.losses:
                if loss == "masks":
                    # Intermediate masks losses are too costly to compute, we ignore them.
                    continue
        ...
        if self.training and ("dn_outputs" in outputs):
            single_pad, num_dn_groups = self.prep_for_dn(outputs["dn_outputs"])
            dn_pos_idx = []
            dn_neg_idx = []
            for i in range(len(targets)):
                if len(targets[i]["labels"]) > 0:
                    t = torch.range(0, len(targets[i]["labels"]) - 1).long().cuda()
                    t = t.unsqueeze(0).repeat(num_dn_groups, 1)  # [num_dn_groups, gt num]
                    tgt_idx = t.flatten()
                    output_idx = (torch.tensor(range(num_dn_groups)) * single_pad).long().cuda().unsqueeze(1) + t

                    output_idx = output_idx.flatten()
                else:
                    output_idx = tgt_idx = torch.tensor([]).long().cuda()
                dn_pos_idx.append((output_idx, tgt_idx))
                dn_neg_idx.append((output_idx + single_pad // 2, tgt_idx)) # useless
            ...
            for loss in self.losses:
                ...
                if "labels" in loss:
                    # Logging is enabled only for the last layer
                    kwargs["log"] = False
                l_dict = self.get_loss(loss, outputs["dn_outputs"], targets, dn_pos_idx, num_boxes * num_dn_groups, **kwargs)
                ...
                for i, aux_outputs in enumerate(outputs["dn_aux_outputs"]):
                    for loss in self.losses:
                        ...
                        if loss == "labels":
                            # Logging is enabled only for the last layer
                            kwargs["log"] = False
                        l_dict = self.get_loss(loss, aux_outputs, targets, dn_pos_idx, num_boxes * num_dn_groups, **kwargs)
                        l_dict = {k + f"_dn_{i}": v for k, v in l_dict.items()}
                        losses.update(l_dict)

        criterion_output = [losses, mat_indices]
        if "aux_outputs" in outputs:
            criterion_output.append(aux_indices)
        if "enc_outputs" in outputs:
            criterion_output.append(enc_indices)
        if "dn_outputs" in outputs:
            criterion_output.append(dn_pos_idx)
