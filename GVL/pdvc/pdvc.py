...
class PDVC(nn.Module):
    ...
    def __init__(
        ...
        """
        super().__init__()
        assert two_stage_mode in ("e2e", "aux", None), f"Unknown two stage mode: {two_stage_mode}"
        ...
        self.dn_labelbook_size = self.num_classes
        self.label_enc = nn.Embedding(self.dn_labelbook_size + 1, self.hidden_dim)

        support_mlp_class_head = vars(opt).get('support_mlp_class_head', False)
        if support_mlp_class_head:
            self.class_head = MLP(self.hidden_dim, self.hidden_dim, self.num_classes, 3)
        else:
            self.class_head = nn.Linear(self.hidden_dim, self.num_classes)
        self.count_head = nn.Linear(self.hidden_dim, opt.max_eseq_length + 1)
        self.bbox_head = MLP(self.hidden_dim, self.hidden_dim, 2, 3)
        ...
        if self.enable_contrastive:
            ...
            if self.opt.enable_multilayer_projection:
                self.contrastive_projection_event = MLP(self.hidden_dim, self.hidden_dim, opt.contrastive_hidden_size, 2)
                self.contrastive_projection_text = MLP(text_encoder_hidden_dim, text_encoder_hidden_dim, opt.contrastive_hidden_size, 2)
            else:
                self.contrastive_projection_event = nn.Linear(self.hidden_dim, opt.contrastive_hidden_size)
        ...
        if self.two_stage_mode:
            self.transformer.decoder.class_head = self.class_head
            for bbox_head in self.bbox_head:
                nn.init.constant_(bbox_head.layers[-1].bias.data[2:], 0.0)
        else:
    ...
    def forward(self, dt, criterion, contrastive_criterion, eval_mode=False):
        ...
        if (self.dn_number > 0) and (dt["video_target"] is not None):
            input_query_label, input_query_bbox, attn_mask, dn_meta = prepare_for_cdn(
                dn_args=(dt["video_target"], self.dn_number, self.dn_label_noise_ratio, self.dn_box_noise_scale),
                training=self.training,
                num_queries=self.transformer.two_stage_num_proposals,
        ...
        if self.two_stage_mode:
            init_reference, tgt, reference_points, query_embed, enc_outputs = self.transformer.prepare_decoder_input_proposal(memory, mask_flatten, temporal_shapes, input_query_bbox, input_query_label)
        else:
        ...
        hs, inter_references = self.transformer.forward_decoder(tgt, reference_points, memory, temporal_shapes,
                                                                ...
                                                                mask_flatten, attn_mask, False)

        references = torch.concat([init_reference.unsqueeze(0), inter_references], dim=0)
        if self.dn_number > 0 and dn_meta is not None:
            hs, references = dn_post_process(hs, references, dn_meta)

            # project to co-embedding space
            if self.enable_contrastive:
                dn_event_embed = torch.stack([self.contrastive_projection_event[i](hs_i) for i, hs_i in enumerate(dn_meta["output"]["hs"])])
            else:
                dn_event_embed = dn_meta["output"]["hs"]
            dn_meta["output"]["event_embed"] = dn_event_embed
        ...
        if eval_mode or self.opt.caption_loss_coef == 0:
            out, loss = self.parallel_prediction_full(
                dt,
                criterion,
                ...
                references,
                others,
                self.opt.eval_disable_captioning,
                enc_outputs
            )
        else:
            if self.opt.set_cost_caption > 0:
                ...
                out, loss = self.parallel_prediction_matched(
                    dt,
                    criterion,
                    contrastive_criterion,
                    hs,
    ...
    def parallel_prediction_full(
        ...
        outputs_cap_probs = []
        outputs_cap_seqs = []
        outputs_cl_match_mats = []

        num_pred = hs.shape[0]
        for l_id in range(num_pred):
            reference = inter_reference[l_id]  # [decoder_layer, batch, query_num, ...]
            hs_lid = hs[l_id]
            ...
            if l_id != num_pred - 1 or disable_captioning:
            ...
            if self.enable_contrastive:
                ...
                assert len(others["text_embed"]) == num_pred, f"visual features have {num_pred} levels, but text have {len(others['text_embed'])}"
                ...
                outputs_cl_match_mats.append(cl_match_mat)
            else:
                outputs_cl_match_mats.append(0)
        ...
        outputs_classes = torch.stack(outputs_classes)  # [decoder_layer, bs, num_query, n_class]
        outputs_counts = torch.stack(outputs_counts)
        outputs_coords = torch.stack(outputs_coords)  # [decoder_layer, bs, num_query, 2]
        all_out = {
            "pred_logits": outputs_classes,
            "pred_count": outputs_counts,
            "pred_boxes": outputs_coords,
            "caption_probs": outputs_cap_probs,
            "seq": outputs_cap_seqs,
            "cl_match_mats": outputs_cl_match_mats,
            "event_feat": hs
        }
        ...
        if enc_outputs is not None:
            enc_count = self.predict_event_num(self.count_head[num_pred], enc_outputs["output_memory"])
            enc_outputs.update({"pred_count": enc_count})
            out["enc_outputs"] = enc_outputs

        if self.aux_loss:
            loss, mat_indices, aux_indices, _ = criterion(out, dt["video_target"])

            for l_id in range(num_pred):
                reference = inter_reference[l_id]
                hs_lid = hs[l_id]
                indices = mat_indices[0] if l_id == num_pred - 1 else aux_indices[l_id][0]
                cap_loss, cap_probs, seq, cost_cap = self.caption_prediction(
                    self.caption_head[l_id],
                    dt,
                    hs_lid,
                    reference,
                    others,
    ...
    def parallel_prediction_matched(
        self,
        dt,
        criterion,
        contrastive_criterion,
        ...
        for l_id in range(num_pred):
            ...
            outputs_count = self.predict_event_num(self.count_head[l_id], hs_lid)
            tmp = self.bbox_head[l_id](hs_lid)  # [bs, num_query, 2]
            _, cap_probs, seq, _ = self.caption_prediction(self.caption_head[l_id], dt, hs_lid, reference.detach(), others, "none")
        ...
        if dn_outputs is not None:
            ...
            outputs_cl_match_mats = []
            for l_id in range(num_pred):
                reference = dn_outputs["output"]["references"][l_id]  # [decoder_layer, batch, query_num, ...]
                hs_lid = dn_outputs["output"]["hs"][l_id]
                outputs_class = self.class_head[l_id](hs_lid)  # [bs, num_query, n_class]
                outputs_count = self.predict_event_num(self.count_head[l_id], hs_lid)
                tmp = self.bbox_head[l_id](hs_lid)  # [bs, num_query, 2]
                _, cap_probs, seq, _ = self.caption_prediction(self.caption_head[l_id], dt, hs_lid, reference.detach(), others, "none")
                ...
                if self.enable_contrastive and self.opt.set_cost_cl > 0:
                    text_embed = torch.cat(others["text_embed"][l_id], dim=0)
                    event_embed = dn_outputs["output"]["event_embed"][l_id]
                    event_embed = event_embed.reshape(-1, event_embed.shape[-1])
                    cl_match_mat = contrastive_criterion.forward_logits(text_embed, event_embed, self.background_embed).t()
                    outputs_cl_match_mats.append(cl_match_mat)
                else:
            ...
            out["dn_outputs"] = {k: v[-1] for k, v in all_out.items()}
            ks, vs = list(zip(*(all_out.items())))
            out["dn_aux_outputs"] = [{ks[i]: vs[i][j] for i in range(len(ks))} for j in range(num_pred - 1)]

            out["dn_outputs"].update(dn_outputs)

        if self.aux_loss:
            loss, mat_indices, aux_indices, _, dn_pos_idx = criterion(out, dt["video_target"])
            ...
            if False:
                for l_id in range(num_pred):
                    ...
                    l_dict = {"loss_caption_dn": cap_loss}
                    if self.enable_contrastive:
                        contrastive_loss = contrastive_criterion(
                            text_embed = others["text_embed"][l_id],
                            event_embed = dn_outputs["output"]["event_embed"][l_id],
                            matching_indices = dn_pos_idx,
                            bg_embed = self.background_embed,
                        )

                        l_dict.update({"contrastive_loss_dn": contrastive_loss})
...
def build(args):
    ...
    if args.aux_loss:
        for i in range(args.dec_layers - 1):
            new_weight_dict.update({k + f'_{i}': v for k, v in weight_dict.items()})
    if args.two_stage_mode:
        new_weight_dict.update({k + f'_enc': v for k, v in weight_dict.items()})
    if args.dn_number > 0:
        new_weight_dict.update({k + f'_dn': v for k, v in weight_dict.items()})
        for i in range(args.dec_layers - 1):
            new_weight_dict.update({k + f'_dn_{i}': v for k, v in weight_dict.items()})
